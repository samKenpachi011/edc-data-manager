...
class ActionItem(BaseModel):
    ...
    action_date = models.DateField(
        verbose_name='action_date',
        default=date.today
    )

    expiration_date = models.DateField(
        default=date.today() + timedelta(days=90),
        help_text='Data note will automatically be set to \'Resolved\' in 90 days unless otherwise specified.'
    )
    ...
    action_group = models.CharField(
        ...
        default='no group',
        help_text=(
            'You can only select a group to which you belong. Choices are based on Groups defined in Auth.')
    )
